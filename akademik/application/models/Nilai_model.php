...
class Nilai_model extends CI_Model {
	...
	public function walikelas_all() {
		$q = $this->db->query("SELECT * FROM vw_walikelas  ORDER BY id_walikelas DESC");
		return $q;
	}
		
	public function jadwal($id_guru) {
		$q = $this->db->query("SELECT * FROM vw_ajar_guru WHERE id_guru = '$id_guru'  ORDER BY id_jadwal_pelajaran DESC");
		return $q;
	}
	
	public function jadwal_all() {
		$q = $this->db->query("SELECT * FROM vw_ajar_guru  ORDER BY id_jadwal_pelajaran DESC");
		return $q;
    }
    
    public function nilai_harian_kategori($id_jadwal_pelajaran) {
		$q = $this->db->query("SELECT * FROM nilai_harian WHERE id_jadwal_pelajaran = $id_jadwal_pelajaran  ORDER BY id_jadwal_pelajaran DESC");
		return $q;
    }
    
    public function nilai_harian_siswa($id_nilai_harian) {
		$q = $this->db->query("SELECT nis,nama_siswa,nilai,id_nilai_harian,id_nilai_harian_detail FROM nilai_harian_detail 
