...
<div style="float:left;width:450px;">
        ...
        ?>
        <img style="float:left;width:80px;height:80px;" src="<?php echo 'http://'.$_SERVER['SERVER_NAME']; ?>/asispanel/upload/<?php echo $logo; ?>">
    ...
    <div style="float:left;">
        ...
        <table>
            ...
            <tr>
                <td style="font-size:14px;">BUKTI PEMBAYARAN : <b>INV.<?php echo date("ymd",strtotime($tanggal)).'.'.$id_siswa; ?></b></td>
            </tr>
        </table>
    </div>
    ...
    <table class="table" cellpadding="5" cellspacing="0">
        <thead>
            <tr>
                ...
                <th class="th" style="text-align:center;">Jumlah Pembayaran</th>
                <th class="th" style="text-align:center;">Sisa Tagihan</th>
            </tr>
        </thead>
        <tbody>
            <?php
            ...
            $pembayaran_bulanan = $this->db->query("SELECT bulan,nama_pos_keuangan,tahun_ajaran,tagihan,bayar,tanggal,tahun_ajaran FROM pembayaran_bulanan 
                    ...
                    INNER JOIN mst_pos_keuangan ON mst_jenis_pembayaran.id_pos_keuangan = mst_pos_keuangan.id_pos_keuangan  WHERE id_siswa = $id_siswa AND tanggal = '$tanggal'");

            $pembayaran_bebas = $this->db->query("SELECT nama_pos_keuangan,tahun_ajaran,tagihan,bayar,tanggal,bayar,pembayaran_bebas.id_pembayaran_bebas FROM pembayaran_bebas_dt 
                    ...
                    INNER JOIN mst_pos_keuangan ON mst_jenis_pembayaran.id_pos_keuangan = mst_pos_keuangan.id_pos_keuangan WHERE id_siswa = $id_siswa AND pembayaran_bebas_dt.tanggal = '$tanggal'");
                    ...
                    <td style="text-align:right;">
                        <?php
                            if($data['bayar'] >= $data['tagihan']) {
                                ...
                                echo 'Rp. '.number_format($data['tagihan']);
                            }
                        ?>
                    </td>
                </tr>
            ...
            foreach ($pembayaran_bebas->result_array() as $data) {
                ...
                $total_bayar += $data['bayar'];
                $seluruh_bayar = $tot_bayar->hitung;
                ?>
                <tr>
                    <td style="text-align:center;"><?php echo $no; ?></td>
                    ...
                    <td style="text-align:right;">
                        <?php
                            if($seluruh_bayar >= $data['tagihan']) {
                                echo 'Lunas';
                            } else {
                                echo 'Rp. '.number_format($data['tagihan'] - $seluruh_bayar);
                            }
                        ?>
                    </td>
                </tr>
            ...
            <tr>
                <td class="tf"></td>
                <td class="tf"></td>
                <td class="tf" style="text-align:left;font-weight:bold;">Total Pembayaran</td>
                <td class="tf" style="text-align:right;font-weight:bold;">Rp <?php echo number_format($total_bayar); ?></td>
                <td class="tf"></td>
            </tr>
            <tr>
                <td class="tf" colspan="4" style="text-align:right;font-style:italic;font-weight:bold;"><?php echo terbilang($total_bayar); ?>  Rupiah</td>
                <td class="tf"></td>
            </tr>
        </tbody>
    </table>
